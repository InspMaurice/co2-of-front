...
export class Co2Component {
  ...
  firstEstimationCo2() {
    console.log("First estimation of CO2 emissions");
    const relevantResources = this.getRelevantResources(0);
    console.log("Relevant resources for first estimation:", relevantResources);
    this.resetState();
    ...
    console.log("ça arrive avant le return");
    return {
      weight: totalBytes,
      co2weight: Number(
  ...
  getCo2byItem(item, greenHosting, options) {
    console.log("Calculating CO2 for item:", item, "Green hosting:", greenHosting, "Options:", options);
    try {
      const oneByte = new window.co2({ model: "1byte" });
